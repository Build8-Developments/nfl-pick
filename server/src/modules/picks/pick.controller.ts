...
export const getMyPickByWeek = async (req: Request, res: Response) => {
  ...
  try {
    ...
    console.log("[PICKS] getMyPickByWeek", { userId, week, found: Boolean(pick) });
  } catch (err) {
...
export const getPickByQuery = async (req: Request, res: Response) => {
  ...
  const requestedUserId = (req.query.userId as string | undefined) || authUserId;
  const weekNum = Number(req.query.week);

  if (!Number.isFinite(weekNum)) {
    return res.status(400).json(ApiResponse.error("Invalid or missing 'week' query param"));
  }
  if (!requestedUserId) {
    return res.status(400).json(ApiResponse.error("Missing 'userId' and no authenticated user"));
  }
  ...
  if (authUserId && requestedUserId !== String(authUserId)) {
    return res.status(403).json(ApiResponse.error("Forbidden: cannot access other user's picks"));
  }
...
export const upsertMyPick = async (req: Request, res: Response) => {
  ...
  const authUser = (req as any).user as { _id: string; role?: string } | undefined;
  if (!authUser?._id) {
  ...
  const providedUserId = (req.query.userId as string | undefined) || (req.body as any)?.userId;
  const targetUserId = authUser.role === "admin" ? (providedUserId || String(authUser._id)) : String(authUser._id);
  const targetUserObjectId = new mongoose.Types.ObjectId(targetUserId);

  const { selections, lockOfWeek, touchdownScorer, propBet, propBetOdds, isFinalized } = req.body as any;
  ...
  try {
    const existing = await Pick.findOne({ user: targetUserObjectId, week: weekNum });
    if (existing && existing.isFinalized) {
      console.warn("[PICKS] upsert denied: already finalized", { userId: targetUserId, week: weekNum });
      return res.status(400).json(ApiResponse.error("Picks already submitted and locked for this week"));
    }
    ...
    if (isFinalized && selections && Object.keys(selections).length > 0) {
      const selectedGameIds = Object.keys(selections).filter((k) => typeof k === "string" && k.length > 0);
      if (selectedGameIds.length > 0) {
        const games = await Game.find({ gameID: { $in: selectedGameIds } }).lean();
        const now = new Date();
        const parseGameDateTime = (gameDate: string | undefined, gameTime: string | undefined) => {
        ...
        for (const g of games) {
          const kickoff = parseGameDateTime((g as any).gameDate, (g as any).gameTime);
          if (now >= new Date(kickoff.getTime() + 15 * 60 * 1000)) {
            console.warn("[PICKS] upsert denied: game started", { userId: targetUserId, week: weekNum, gameId: (g as any).gameID });
            return res.status(400).json(ApiResponse.error("One or more selected games have started; cannot submit for those games."));
          }
        }
      }
    }
    ...
    if (isFinalized) {
      ...
      const weekGames = await Game.find({ gameWeek: new RegExp(`\\b${weekNum}\\b`, "i") }).lean();
      const now = new Date();
      const parseGameDateTime = (gameDate: string | undefined, gameTime: string | undefined) => {
        if (!gameDate || !gameTime) return new Date(8640000000000000);
        const yyyy = Number(gameDate.slice(0, 4));
      ...
      if (weekGames.length === 0) {
        console.warn("[PICKS] upsert: no games found for week", { week: weekNum });
        return res.status(400).json(ApiResponse.error("No games found for this week"));
      }
      if (selectionCount !== requiredCount) {
        ...
        console.warn("[PICKS] upsert denied: missing selections", { requiredCount, providedCount: selectionCount, missingGameIds });
        return res
          .status(400)
          .json(
            ApiResponse.error(
      ...
      if (invalidIds.length) {
        console.warn("[PICKS] upsert denied: invalid game ids (already started)", { invalidIds });
        return res
          .status(400)
          .json(
            ApiResponse.error(
      ...
      if (!lockOfWeek || typeof lockOfWeek !== "string" || lockOfWeek.trim().length === 0) missingFields.push("lockOfWeek");
      if (!touchdownScorer || typeof touchdownScorer !== "string" || touchdownScorer.trim().length === 0) missingFields.push("touchdownScorer");
      if (!propBet || typeof propBet !== "string" || propBet.trim().length === 0) missingFields.push("propBet");
      if (!propBetOdds || typeof propBetOdds !== "string" || propBetOdds.trim().length === 0) missingFields.push("propBetOdds");
      if (missingFields.length) {
        console.warn("[PICKS] upsert denied: missing fields", { missingFields });
        return res
          .status(400)
          .json(
      ...
      if (!selectedTeams.has(lockOfWeek)) {
        console.warn("[PICKS] upsert denied: lock not in selections", { lockOfWeek });
        return res
          .status(400)
          .json(
            ApiResponse.error(
    ...
    if (!finalSaved) {
      console.log("[PICKS] findOneAndUpdate returned null, trying findOne", { targetUserId, week: weekNum });
      finalSaved = await Pick.findOne({ user: targetUserObjectId, week: weekNum }).lean();
    }
    ...
    try {
      console.log("[PICKS] upsert saved (post-read)", {
        ...
        selectionsCount: finalSaved ? Object.keys(finalSaved.selections || {}).length : 0,
        savedDoc: finalSaved ? JSON.stringify(finalSaved, null, 2) : "null",
      });
    } catch {}
    if (finalSaved?.isFinalized) {
      broadcastLiveEvent({ type: "pick:finalize", payload: { userId: targetUserId, week: weekNum } });
    } else {
      broadcastLiveEvent({ type: "pick:update", payload: { userId: targetUserId, week: weekNum } });
    }
    console.log("[PICKS] About to return response with data:", finalSaved ? "HAS DATA" : "NULL DATA");
    console.log("[PICKS] Final saved document:", JSON.stringify(finalSaved, null, 2));
    ...
    if (err && err.code === 11000) {
      console.warn("[PICKS] upsert duplicate key, retrying as update", { userId: targetUserId, week: weekNum });
      ...
      const savedRetry = await Pick.findOne({ user: targetUserObjectId, week: weekNum }).lean();
      return res.status(200).json(ApiResponse.success(savedRetry, "Picks saved"));
    } else {
      console.error("[PICKS] upsert error", { userId: targetUserId, week: weekNum, err });
      throw err;
    }
  }
  // Fallback (shouldn't generally reach here due to early returns)
  const updated = await Pick.findOne({ user: targetUserObjectId, week: weekNum }).lean();
...
export const deleteMyPick = async (req: Request, res: Response) => {
  ...
  if (existing && existing.isFinalized) {
    return res.status(400).json(ApiResponse.error("Cannot delete a submitted pick for this week"));
  }
...
export const getAllPicksByWeek = async (req: Request, res: Response) => {
  ...
  try {
    picks = await Pick.find({ week, isFinalized: true })
      ...
      .sort({ updatedAt: -1 })
      .lean();
    console.log("[PICKS] getAllPicksByWeek", { week, count: picks?.length || 0 });
  } catch (err) {
    console.error("[PICKS] getAllPicksByWeek error", { week, err });
  }
  // Compute outcomes based on Game winner when available
  const games = await Game.find({ gameWeek: new RegExp(`\\b${week}\\b`, "i") }).lean();
...
export const getWeeksWithFinalizedPicks = async (req: Request, res: Response) => {
