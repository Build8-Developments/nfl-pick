...
import { getUserAvatar, preloadAvatars, batchComputeAvatarUrls } from '../lib/avatarUtils';
...
export const useAvatarService = () => {
  ...
  const [preloadedAvatars, setPreloadedAvatars] = useState<Set<string>>(new Set());

  /**
   * Get avatar URL with caching
   */
  const getAvatar = useCallback((avatar?: string, fallback?: string): string => {
   ...
   */
  const preloadAvatarsBatch = useCallback(async (avatars: (string | undefined)[]): Promise<void> => {
    const uniqueAvatars = [...new Set(avatars.filter(Boolean))];
    const notPreloaded = uniqueAvatars.filter(avatar => !preloadedAvatars.has(avatar!));
    
    if (notPreloaded.length === 0) return;
    
    try {
      await preloadAvatars(notPreloaded);
      setPreloadedAvatars(prev => new Set([...prev, ...notPreloaded]));
   ...
   */
  const getAvatars = useCallback((avatars: (string | undefined)[]): Map<string, string> => {
...
export const useUserAvatars = (users: Array<{ avatar?: string }>) => {
  ...
  const getUserAvatar = useCallback((avatar?: string) => {
