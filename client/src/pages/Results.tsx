  ...
  useEffect(() => {
    const init = async () => {
      try {
        ...
        if (!cachedTeams) {
          const res = await apiClient.get<{ success: boolean; data?: ITeam[] }>("teams");
          const list = Array.isArray(res.data) ? res.data : [];
          setTeams(list);
          memCache.set("teams", list);
        }
        if (!cachedGames) {
          const res = await apiClient.get<{ success: boolean; data?: IGame[] }>("games");
          const list = Array.isArray(res.data) ? (res.data as IGame[]) : [];
          setGames(list);
          memCache.set("games", list);
        }
        const finalWeeksRes = await apiClient.get<{ success: boolean; data?: number[] }>("picks/weeks");
        const weeks = Array.isArray(finalWeeksRes.data) ? finalWeeksRes.data : [];
        const gameWeeks = (cachedGames || [])
          .map((g) => (typeof g.gameWeek === "string" ? g.gameWeek.match(/\d+/)?.[0] : undefined))
          .filter((n): n is string => Boolean(n))
          .map((n) => Number(n))
          .filter((n) => !Number.isNaN(n));
        const merged = [...new Set([...(weeks || []), ...(gameWeeks || [])])].sort((a, b) => a - b);
      ...
      <div className="flex items-center justify-between">
        ...
        {availableWeeks.length > 0 && (
          <div className="flex items-center gap-2">
            ...
            <Select value={selectedWeek?.toString() || ""} onValueChange={(v) => {
              const n = Number(v);
              setSelectedWeek(n);
            ...
            <Button variant="outline" onClick={() => {
              ...
              apiClient
                .get<{ success?: boolean; data?: BackendPick[] }>(`picks/all/${selectedWeek}`)
                .then((res) => setPicks(Array.isArray(res?.data) ? res.data! : []))
                .catch(() => setPicks([]))
      ...
      <Card>
        ...
        <CardContent>
          {loading ? (
            ...
            <div className="text-center py-8 text-muted-foreground">No picks found for this week</div>
          ) : (
            <div className="space-y-4">
              {picks.map((p) => {
                const username = typeof p.user === "string" ? p.user : p.user?.username || "User";
                return (
                  <div key={p._id} className="p-4 border rounded-lg">
                    <div className="flex items-center justify-between mb-2">
                      <div className="font-semibold">{username}</div>
                      <div className="text-xs text-muted-foreground">Lock: {p.lockOfWeek || "—"}</div>
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2">
                      {Object.entries(p.selections || {}).map(([gid, team]) => (
                        <div key={gid} className="text-sm flex items-center justify-between border rounded-md px-3 py-2">
                          <span className="truncate mr-3">{gameIdToLabel.get(String(gid)) || gid}</span>
                          <span className="font-medium">{team}</span>
                        </div>
                      ))}
                    </div>
                    <div className="mt-2 text-sm text-muted-foreground">
                      TD Scorer: {p.touchdownScorer || "—"} • Prop: {p.propBet || "—"} ({p.propBetOdds || "—"})
                    </div>
                  </div>
                );
              })}
